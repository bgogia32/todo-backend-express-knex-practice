...
async function updateTask(task_id, name, description, duedate, status_id, project_id, iscompleted, assignedUsers) {
    return await knex.transaction(async (trx) => {
        try
        {
            const updatedTasks = await trx('task').where({id: task_id})
                                .update({name: name, description: description, duedate: duedate, status_id: status_id, project_id: project_id, iscompleted: iscompleted, updated_at: knex.fn.now(), updated_by: 1})
                                .returning('*');
            const updatedUsers = await AssignUsersToTask(trx, assignedUsers, task_id, 'Update');
...
async function ValidateProjectUsers(user_ids, project_id) {
    ...
    return userAssignments;
        ...
        return {error: 'Unable to validate users.'};
    }
...
async function AssignUsersToTask(trx, assignedUsers, task_id, assignmentType = 'Create') {
    try
    {
        ...
        console.error('Assigning failed:', error);
        return {error: 'Unable to assign users.'};
    }
}
